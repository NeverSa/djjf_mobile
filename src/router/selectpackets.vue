...
export default {
  ...
  methods: {
    gotoinvest(item){
        this.$router.push({path: '/investmentconform', query: {id:item.id,amount:item.voucherValue/100,productid:this.$route.query.productid,money:this.$route.query.amount}})
    },
    getdetail(){
      this.$http.get(`${this.Interface.NodeServer}/user/${this.userId}/account/canusevouchers/${this.$route.query.productid}?type=0&page=1&limit=100&amount=${this.$route.query.amount*100}&token=${this.sessionid}&from=android&version=100`).then(res=>{
