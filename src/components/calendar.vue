    ...
    <div class="calendar">
        <div class="calendar-tools">
            ...
            <div class="calendar-info">
                ...
                <div class="year">{{year}}年<span>{{month+1}}月</span></div>
            </div>
        </div>
        <table cellpadding="5">
        <thead>
        ...
        <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}">
            <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(k1,k2,$event,child.disabled,child.eventName)">
                <span >{{child.day}}</span>
                <div class="text roude" v-if="child.eventName!=undefined"></div>
...
export default {
    ...
    methods: {
        ...
        // 选中日期
        select(k1, k2, e,dis,name) {
            if (e != undefined) e.stopPropagation()
                // 日期范围
                // if(dis==false||(dis==true&&name)){
